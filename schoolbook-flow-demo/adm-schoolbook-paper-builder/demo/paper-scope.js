(() => {
  ...
  const valid = paper => !!province(paper?.province) && cities(paper.province).some(c=>c.id===paper.city) && stages.includes(paper.stage);
  ...
  const label = paper => `${regionLabel(paper)} · ${stages.includes(paper?.stage)?paper.stage:'学段待补充'}`;
