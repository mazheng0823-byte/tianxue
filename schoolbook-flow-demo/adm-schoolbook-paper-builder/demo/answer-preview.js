(() => {
  ...
  function rulesForItems(items, stored = {}, scope={}) {
    return Object.fromEntries(items.flatMap((q,i) => {
      ...
      const matchesScope=saved?.scope ? PaperScope.key(saved.scope)===PaperScope.key(scope) : !!template && template.province===scope.province && template.stage===scope.stage && (template.city==='*'||template.city===scope.city);
  ...
  function saveRule() {
    ...
    else p.flowOverrides[currentQuestion().id] = {...clone(editor),scope:PaperScope.from(p),custom:changed};
